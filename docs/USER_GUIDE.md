...
- **ประวัติการเข้าสู่ระบบ**: ทุกความพยายาม login (สำเร็จ/ล้มเหลว/ถูกจำกัดอัตรา) ถูกบันทึกใน DB — กันการสุ่มรหัสผ่าน; จำกัด 5 ครั้ง/นาทีต่อ IP + 30 ครั้งรวมทุก IP (ตั้งได้ใน `config.toml [auth]`)
- **เปลี่ยนรหัสผ่าน**: กรอกรหัสผ่านปัจจุบัน + ใหม่ 2 ครั้ง → บันทึก (มีผลทันที ไม่ restart); ครั้งแรก exe สร้าง user/pass อัตโนมัติให้auto-fill ในหน้า login (เห็นช่องกรอกแล้ว) — แนะนำเปลี่ยนทันทีหลังเข้าสู่ระบบ
- **การแจ้งเตือน (Webhook / Telegram)** — ตั้งค่าช่องทางรับการแจ้งเตือน alert ผ่าน UI (บันทึกแล้วมีผลทันที ไม่ต้อง restart):
  ...
  - **Telegram**: กรอก **Bot Token** (จาก @BotFather) → กด **"ดึง Chat ID อัตโนมัติ"** (ได้ chat_id เอง ถ้าเคยแชทกับบอท ≥ 1 ครั้ง) หรือกรอกเอง → ปุ่ม **ทดสอบส่งข้อความ** → **บันทึก**; มีตัวช่วย "วิธีหา Bot Token / Chat ID" (คลิกขยาย — Bot Token ต้องสร้างเองผ่าน @BotFather)
  - ติ๊ก **เปิดใช้งาน** เพื่อเปิดช่องทาง (**ไม่เลือก = ตั้งค่าไว้แต่ปิดใช้งาน** — บันทึกแล้วมีข้อความแจ้ง); badge แสดงสถานะ: `พร้อมใช้งาน` / `ปิดใช้งาน` / `ยังไม่ได้ตั้งค่า`
