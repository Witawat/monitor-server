...
- [x] Auth/security: API token ต่อ host (X-Agent-Token), rate limit ingest + login (DB-based ต่อ IP + รวมทุก IP กัน botnet, ตั้งค่าได้ใน `[auth]`), audit log login (สำเร็จ/ล้ม/ถูกจำกัด, ดูในหน้า ตั้งค่า), เปลี่ยนรหัสผ่านผ่าน UI (DB hash > config, ไม่ restart), auto-fill user/pass ครั้งแรก (exe สร้าง), CSP headers
